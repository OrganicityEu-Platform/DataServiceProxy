...
import config.PatraConfiguration;
...
@Service
public class PatraAPIService {
    // call example
    // http://195.220.224.231:8091/api/v1/devices/urn:oc:entity:patra:weatherstation:fixed:1087159755/readings?attribute_id=type:urn:oc:attributeType:relativeHumidity&from=2015-10-01T00:57Z&to=2015-12-02T11:57Z&function=avg

    @Autowired
    PatraConfiguration patraConfiguration;

    public PatraAPIService() {
    }

    public PatraAPIService(PatraConfiguration ac) {
        this.patraConfiguration = ac;
    }
    ...
    public SmartphoneData getData(String entity_id, String attribute_id, String start, String end, String rollup, String function, String offset, String limit) throws Exception {
        ...
        String url = patraConfiguration.getSensorDataEndpoint() + "/" + entity_id + "/readings?attribute_id=" + attribute_id;
        url += "&from=" + start;
        url += "&to=" + end;
        if (limit != null)
            url += "&limit=" + limit;
